...
class DeepNetwork:
    ...
    @staticmethod  
    def build(env, params, index, actor=False, name='model'):
        ...
        input_size = env.observation_space[index].shape[0]
        action_size = env.action_space[index].n
        ...
        for i in range(h_layers):
            h = Dense(h_size, activation='tanh', name='hidden_' + str(i))(h)

        if actor:
            y = Dense(action_size, activation='tanh', name='output_layer')(h)  
        else:
            y = Dense(1, activation='tanh', name='critic_output_layer')(h)
